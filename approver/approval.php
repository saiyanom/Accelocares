...
*/
	...
	if ($session->is_employee_logged_in()) { 				
		...
		foreach ($employee_loc as $employee_loc) {
			//echo $employee_loc->emp_sub_role.", ";	
			if($employee_loc->emp_sub_role == "CRM - Head" || $employee_loc->emp_sub_role == "Commercial Head" || $employee_loc->emp_sub_role == "Plant Chief - AN" 
			   || $employee_loc->emp_sub_role == "Sales Head" || $employee_loc->emp_sub_role == "CFO" || $employee_loc->emp_sub_role == "MD"){
				$emp_loc_num++; break;
			} else {
...
<div class="alert fade show text-white <?php 
  ...
  else if($message == "Approval note updated successfully."){echo "bg-success";} 		
	else {echo "bg-danger";}?>" 
...
<table id="my-complaints" class="table dt-responsive basic-datatable" width="100%">
	<thead>
		<tr>
			<td>Complaint No</td>
			<td>Company Name</td>
			...
			if($num > 0){
				...
				foreach ($employee_loc as $employee_loc) {
					if($employee_loc->emp_sub_role == "CRM - Head" || $employee_loc->emp_sub_role == "Commercial Head" || $employee_loc->emp_sub_role == "Plant Chief - AN" || $employee_loc->emp_sub_role == "Sales Head"  || $employee_loc->emp_sub_role == "CFO" || $employee_loc->emp_sub_role == "MD" ){
						...
						if($employee_loc->emp_sub_role == "MD"){
							$comp_sql = "Select * from complaint where approval_note != '' AND {$company} {$status} {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} cna_md_status = 'Yes' AND date_ != '0000-00-00' order by id DESC";
						} else {
							$comp_sql = "Select * from complaint where approval_note != '' AND {$company} {$status} {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} date_ != '0000-00-00' order by id DESC";
						}
					}
				}
				...
				foreach ($employee_loc as $employee_loc) {
					if($employee_loc->emp_sub_role == "CRM - Head" || $employee_loc->emp_sub_role == "Commercial Head" || $employee_loc->emp_sub_role == "Plant Chief - AN" || $employee_loc->emp_sub_role == "Sales Head"  || $employee_loc->emp_sub_role == "CFO" || $employee_loc->emp_sub_role == "MD" ){
						...
						if($employee_loc->emp_sub_role == "MD"){
							$comp_sql = "Select * from complaint where {$status} cna_md_status = 'Yes' AND approval_note != '' order by id DESC";
						} else {
							$comp_sql = "Select * from complaint where {$status} approval_note != '' order by id DESC";
						}						
					} 
				}
			}			
				...
				foreach($emp_location as $emp_location){
					...
					if($product_id->id == $emp_location->product_id){
						...
						if($emp_sub_role_appr == "CRM - Head"){
							if(empty($complaint->cna_crm_head)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Commercial Head"){
							if(empty($complaint->cna_commercial_head)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Plant Chief - AN"){
							if(empty($complaint->cna_plant_chief)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Sales Head"){
							if(empty($complaint->cna_sales_head)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "CFO"){
							if(empty($complaint->cna_cfo)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "MD"){
							if(empty($complaint->cna_md)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						}

						//echo $app_status." - {$_GET['status']}, ";
						//echo $app_status." - {$_GET['status']} , ";


						if(isset($_GET['status']) && !empty($_GET['status']) && $_GET['status'] == $app_status){
							//echo "- Yes -,";
							if(isset($_GET['upper_aging'])){
								if($aging <= $upper_aging && $aging >= $lower_aging){	
									...
									getApprovalPageCommonData($complaint,$emp_sub_role_appr);
