<?php
	...
	if ($session->is_employee_logged_in()) { 				
		...
		foreach ($employee_loc as $employee_loc) {	
			...
			if($employee_loc->emp_sub_role == "Quality Assurance" || $employee_loc->emp_sub_role == "Plant Head" || $employee_loc->emp_sub_role == "Plant Chief - CN"
			   || $employee_loc->emp_sub_role == "Management Representative"){
				$emp_loc_num++; break;
			} else {
	...
	<tbody>
		...
		if(isset($_GET['company'])){
			...
			if($num > 0){
				...
				$employee_loc = EmployeeLocation::find_by_sql($sql);
				foreach ($employee_loc as $employee_loc) {
					if($employee_loc->emp_sub_role == "Quality Assurance" || $employee_loc->emp_sub_role == "Plant Head" || $employee_loc->emp_sub_role == "Plant Chief - CN" || $employee_loc->emp_sub_role == "Management Representative" ){
			...
			if($product_id){
				...
				foreach($emp_location as $emp_location){
					if($product_id->id == $emp_location->product_id){
						...
						if($emp_sub_role_appr == "Quality Assurance"){
							if(empty($complaint->capa_qa)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Plant Head"){
							if(empty($complaint->capa_ph)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Plant Chief - CN"){
							if(empty($complaint->capa_pc)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						} else if($emp_sub_role_appr == "Management Representative"){
							if(empty($complaint->capa_mr)){ $app_status = "Open"; } else { $app_status = "Closed"; }
						}

						//echo $app_status." - {$_GET['status']} - {$emp_sub_role_appr}, ";
