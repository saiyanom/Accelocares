...
<tbody>
	<?php
		...
		if(isset($_GET['company'])){
			...
			//echo $num."<hr />";
			if($num > 0){
				$sql = "Select * from complaint where {$company} {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} date_ != '0000-00-00'  order by id DESC";
		...
		foreach($complaint as $complaint){
			...
			if($product_id){
				$emp_sql = "Select * from employee_location where product_id = '{$product_id->id}' AND emp_id = '{$employee_reg->id}' AND emp_sub_role != 'Employee' AND emp_sub_role != 'Viewer'";
						...
						if(isset($_GET['upper_aging'])){
							if($aging <= $upper_aging && $aging >= $lower_aging){
								...
								echo "<td><form method='post' action='reassign-employee-db.php?id={$complaint->id}' autocomplete='off'><select style='width: 70%; float-left; display:inline' class='form-control assign_emp select_{$num} select2' data-toggle='select2' id='employee_{$num}' name='employee'>";
									$emp_loc_sql 		= "Select * from employee_location where product_id = '{$product_id->id}' order by emp_name ";
									$employee_reg_loc 	= EmployeeLocation::find_by_sql($emp_loc_sql);
									$last_emp = "";
									foreach($employee_reg_loc as $employee_reg_loc){
										if($last_emp != $employee_reg_loc->emp_email){
											echo "<option ";
											if($employee_reg_loc->emp_id == $complaint->emp_id){ echo "Selected";}
											echo " value='{$employee_reg_loc->emp_id}'>{$employee_reg_loc->emp_name}, {$employee_reg_loc->emp_email}</option>";
											$last_emp = $employee_reg_loc->emp_email;
										} 
