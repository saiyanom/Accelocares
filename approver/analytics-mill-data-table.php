		...
		if(isset($_GET['date_range'])){
			...
			if($num > 0){
				...
				foreach($employee_location as $employee_location){
					...
					$sql_mill 	= "Select * from complaint where {$date_range} mill = '{$mill_reg->mill_name}' AND plant_location = '{$product->site_location}' AND business_vertical = '{$product->department}' AND product = '{$product->product}'";
