...
<head>
<meta charset="utf-8" />
<title>Business Vertical | Mahindraaccelo</title>
			...
			<ul class="form-block-con">
				<li>
					<select id="business_vertical" name="business_vertical" class="form-control">
						...
						<option <?php if(isset($_GET['business_vertical']) && !empty($_GET['business_vertical'])){if($_GET['business_vertical'] == "Auto"){ echo "Selected";}} else{ echo "Selected";}?>>Auto</option>
...
<div class="card-body">
<div class="al-com">
<div class="compl-lef"><h4>Business Vertical</h4></div>
...
<thead>
	<tr>
		...
		<?php 
			...
			if(isset($_GET['business_vertical']) && !empty($_GET['business_vertical'])){
				$b_vertical = $_GET['business_vertical'];
			} else { $b_vertical = "Auto"; }
		
			$sql = "Select * from site_location where department = '{$b_vertical}' order by site_location ASC";
			$site_location = SiteLocation::find_by_sql($sql);
			$repeat = "";
			foreach($site_location as $site_location){
				if($repeat != $site_location->site_location){
					echo "<th>{$site_location->site_location} No. of Compl.  Received </th>";
					echo "<th>{$site_location->site_location} No. of Compl.  Closed</th>";
...
<?php
	...
	foreach($site_location as $site_location){
		if($repeat != $site_location->site_location){
			...
			if(isset($_GET['business_vertical']) || isset($_GET['date_range'])){
				...
				if($num > 0){
					...
					foreach($employee_location as $employee_location){
						...
						$sql = "Select * from complaint where plant_location = '{$product->site_location}' AND {$business_vertical} {$date_range} date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);
						
						foreach($complaint as $complaint){   
							if($complaint->mill == $mill_reg->mill_name && $complaint->business_vertical == $b_vertical && $complaint->plant_location == $site_location->site_location){
								$total_complaint_received++;
							}
							if($complaint->mill == $mill_reg->mill_name && $complaint->status == "Closed" && $complaint->business_vertical == $b_vertical && $complaint->plant_location == $site_location->site_location){
					...
					foreach($employee_location as $employee_location){
						$product = Product::find_by_id($employee_location->product_id);
						$sql = "Select * from complaint where plant_location = '{$product->site_location}' AND date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);
			...
			echo "<td>{$total_complaint_closed}</td>";
	} $repeat = $site_location->site_location;} 
?>
	</tr>
<?php } ?>
			...
			if(isset($_GET['business_vertical']) || isset($_GET['date_range'])){
				...
				if($num > 0){					
					...
					foreach($employee_location as $employee_location){
						...
						$product = Product::find_by_id($employee_location->product_id);
						$sql = "Select * from complaint where identify_source = 'Plant' AND plant_location = '{$product->site_location}' AND {$business_vertical} {$date_range} date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);

						foreach($complaint as $complaint){   
							if($complaint->business_vertical == $b_vertical && $complaint->plant_location == $site_location->site_location){
								$total_complaint_received++;
							}
							if($complaint->status == "Closed" && $complaint->business_vertical == $b_vertical && $complaint->plant_location == $site_location->site_location){
								$total_complaint_closed++;
							}
						}
					}
					...
					foreach($employee_location as $employee_location){
						...
						$product = Product::find_by_id($employee_location->product_id);
						$sql = "Select * from complaint where identify_source = 'Plant' AND plant_location = '{$product->site_location}' AND date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);
		...
		<?php 
			...
			foreach($site_location as $site_location){
				if($repeat != $site_location->site_location){
					...
					if(isset($_GET['business_vertical']) || isset($_GET['date_range'])){
						...
						if($num > 0){
							...
							foreach($employee_location as $employee_location){
								...
								$product = Product::find_by_id($employee_location->product_id);
								$sql = "Select * from complaint where identify_source != '' AND plant_location = '{$product->site_location}' AND {$business_vertical} {$date_range} date_ != '0000-00-00' order by id DESC";
								$complaint = Complaint::find_by_sql($sql);
					...
					echo "<td><strong>{$total_total_complaint_received}</strong></td>";
					echo "<td><strong>{$total_total_complaint_closed}</strong></td>";
				} 
				$repeat = $site_location->site_location; 
				$total_total_complaint_received = 0;
				$total_total_complaint_closed = 0;} 
