			...
			echo "data: [";
				...
				if(isset($_GET['from_date']) && !empty($_GET['from_date']) && isset($_GET['to_date']) && !empty($_GET['to_date'])){
					...
					foreach($employee_location as $employee_location){
						...
						$complaint_sql = "SELECT * FROM complaint WHERE plant_location = '{$product->site_location}' AND complaint_type = 'Other' AND date_ BETWEEN '" . $_GET['from_date'] . "' AND  '" . $_GET['to_date']. "'";
						$complaints 	= Complaint::find_by_sql($complaint_sql);
						foreach($complaints as $complaints){ $complaint_type_++;}
					}
