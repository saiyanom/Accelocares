		...
		if($repeat != $site_location->site_location){
			...
			foreach($approval_note as $approval_note){   
				...
				foreach($employee_location as $employee_location){
					...
					$sql_mill 	= "Select * from complaint where id = '{$approval_note->complaint_id}' AND plant_location = '{$product->site_location}' AND business_vertical = '{$product->department}' AND product = '{$product->product}' Limit 1";
					$complaint 		= Complaint::find_by_sql($sql_mill);
