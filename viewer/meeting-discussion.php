...
<div class="form-group" id="discussions">
	...
	<p <?php if($cvc_meeting->status != 0){ echo "style='display:none;'";}?> class="colost2 paddi5">OR</p>

	<textarea <?php if($cvc_meeting->status != 0){ echo "style='display:none;'";}?> class="form-control form-white" name="discussion"/></textarea>
	<p <?php if($cvc_meeting->status == 0){ echo "style='display:none;'";}?>><?php 
		//echo $cvc_meeting->discussion; 
		if(empty($cvc_meeting->discussion_document)){
			echo $cvc_meeting->discussion;
		} else {
			echo "<a target='_blank' class='fancybox-thumbs' data-fancybox-group='thumb' href='../document/cvc/{$cvc_meeting->emp_id}/{$cvc_meeting->discussion_document}'>View Document</a>";
		}
