...
<head>
<meta charset="utf-8" />
<title>Complaint Type Mill | Mahindraaccelo</title>
...
<div class="card-body">
<div class="al-com">
<div class="compl-lef"><h4>Complaint Type Mill</h4></div>
			...
			if(isset($_GET['business_vertical']) || isset($_GET['date_range'])){
				...
				if($num > 0){
					...
					foreach($employee_location as $employee_location){
						...
						$sql = "Select * from complaint where mill = '{$mill_reg->mill_name}' AND plant_location = '{$product->site_location}' AND {$business_vertical} {$date_range} date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);
					...
					foreach($employee_location as $employee_location){
						...
						$product = Product::find_by_id($employee_location->product_id);
						$sql = "Select * from complaint where mill = '{$mill_reg->mill_name}' AND plant_location = '{$product->site_location}' AND date_ != '0000-00-00' order by id DESC";
						$complaint = Complaint::find_by_sql($sql);
					...
					if(isset($_GET['business_vertical']) || isset($_GET['date_range'])){
						...
						if($num > 0){
							...
							foreach($employee_location as $employee_location){
								...
								foreach($complaint as $complaint){   
									if(!empty($complaint->mill) && $complaint->complaint_type == $complaint_type->complaint_type){
										$total_complaint_received++;
									}
									if(!empty($complaint->mill) && $complaint->complaint_type == 'Other'){
										$total_complaint_other++;
									}
								}
							}
