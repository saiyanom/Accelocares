<?php 
	...
	$comp_sql = "Select * from complaint where approval_note != ''  order by id DESC";
?>
<!DOCTYPE html>
<html lang="en">
...
<table id="basic-datatable-1" class="table dt-responsive basic-datatable" width="100%">
	<thead>
		<tr>
			<th>Complaint No</th>
			<th>Company Name</th>
	...
	<tbody>
		...
		if(isset($_GET['company'])){
			...
			//echo $num."<hr />";
			if($num > 0){
				$comp_sql = "Select * from complaint where approval_note != '' AND {$company} {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} date_ != '0000-00-00' order by id DESC";
		...
		foreach($complaint as $complaint){
			...
			if(isset($_GET['upper_aging'])){
				if($aging <= $upper_aging && $aging >= $lower_aging){	
					...
					if(empty($complaint->cna_crm_head) || empty($complaint->cna_commercial_head) || empty($complaint->cna_plant_chief) || empty($complaint->cna_sales_head) || empty($complaint->cna_cfo)){
						...
						if($complaint->cna_md_status == "Yes" && empty($complaint->cna_md)){
							echo "<td><span class='closed-td'>Pending</span></td>";
						} else {
...
<script type="application/javascript">	
	...
	$(document).ready(function() {
      ...
      "lengthMenu": [[5, 10, 20, -1], [5, 10, 20, "All"]],
		...
		$(document).on('click','.view_approval_note',function(){
