...
<?php
	...
	if(isset($_GET['emp_name']) || isset($_GET['meet_status']) || isset($_GET['date_range'])){
		...
		if(isset($session->super_admin_id)){		
			$employee_reg = EmployeeReg::find_by_id($session->super_admin_id);
			if ($employee_reg){ 
				$product_id = '';
				$emp_email = '';
				$num = 1;
				$emp_loc_sql = "Select * from employee_location where emp_id = {$employee_reg->id} ";
