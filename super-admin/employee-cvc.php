...
<head>
<meta charset="utf-8" />
<title>Mahindraaccelo Employee CVC</title>
					...
					<?php
						...
						foreach($employee_reg as $employee_reg){
							...
							if(isset($_GET['emp_name']) && !empty($_GET['emp_name'])){
								if($_GET['emp_name'] == $employee_reg->id){ echo "Selected";}
							}
							echo " value='{$employee_reg->id}'>{$employee_reg->emp_name}</option>";
						}
					?>
				</select></li>
				...
				<li>
					<div class="input-group">
						...
						<div class="custom-file">
							<input readonly type="text" value="<?php if(isset($_GET['date_range'])){ echo $_GET['date_range'];} ?>" id="date_range" name="date_range" placeholder="Select Date Range" class="form-control date" style="padding: 0 5px;" data-provide="datepicker" data-toggle="date-picker" data-single-date-picker="false">
						</div>
					</div>
				</li>
				<li>
...
<div class="card-body">
<div class="al-com">
<div class="compl-lef"><h4>Employee CVC</h4></div>
<div class="clerfix"></div>
</div>
...
<thead>
	<tr>
		...
		<th style="display: none;">Meeting Discussion</th>
		<th>Action</th>
	</tr>
</thead>
<tbody>
...
?>
	...
	<td><?php if($comp_meet->meeting_status == "Yes"){
		...
		} else { echo $comp_meet->remark;} ?>
	</td>
	<td style="display: none;"><?php if($comp_meet->meeting_status == "Yes"){
			echo $comp_meet->discussion; 
		} else { echo $comp_meet->remark;} ?>
	</td>
...
<script type="application/javascript">	
	...
	$(document).ready(function() {
		...
		var date_range_val = '<?php if(isset($_GET['date_range']) && empty($_GET['date_range'])){ echo ""; } else { echo $_GET['date_range']; } ?>';

		$("#date_range").val(date_range_val);
