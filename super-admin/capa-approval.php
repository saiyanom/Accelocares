<?php ob_start();
	...
	foreach ($_POST as $key => $value) {
		if (preg_match('/[\'"\^*()}!{><;`=]/', $value)){
			$session->message("Remove Special Character from Search Form"); redirect_to("capa-approval.php");
		} 
	}
	...
	$comp_sql = "Select * from complaint where capa != ''";
?>
<!DOCTYPE html>
<html lang="en">
...
<table id="basic-datatable" class="table dt-responsive basic-datatable" width="100%">
	...
	<tbody>
		<?php 
			...
			//echo $num."<hr />";
			if($num > 0){
				$comp_sql = "Select * from complaint where capa != '' OR capa_document != '' AND {$company} {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} date_ != '0000-00-00' order by id DESC";
			} else {
				$comp_sql = "Select * from complaint where capa != '' OR capa_document != '' order by id DESC";
			}			
		} else {
			$lower_aging = 0;
			$upper_aging = 20;
		...
		foreach($complaint as $complaint){
			...
			if(isset($_GET['upper_aging'])){
				if($aging <= $upper_aging && $aging >= $lower_aging){
					...
					if(empty($complaint->capa_qa) || empty($complaint->capa_ph) || empty($complaint->capa_pc) || empty($complaint->capa_mr)){
