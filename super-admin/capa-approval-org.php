<?php
	...
	} else if($employee_loc->emp_role == "Approver" && $employee_loc->emp_sub_role == "Plant Chief"){
		$comp_sql = "Select * from complaint where capa != ''";
			...
			<?php 
				...
				if($employee_loc->emp_role == "Approver" && $employee_loc->emp_sub_role == "Quality Assurance"){
					if(empty($complaint->capa_qa)){
						...
						echo "<td><a data-toggle='modal' class='view_capa' data-id='$complaint->capa' data-target='#capa-note' href='#'>View</a></td>";
					} else {
...
<script>
	$('.view_capa').click(function() {
		...
		$("#capa-note .modal-content").load("view-capa.php?id="+data_id);
	});
</script>	
</body>
</html>
