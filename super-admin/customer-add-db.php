<?php 
	...
	foreach ($_POST as $key => $value) {
		if (preg_match('/[\"\^%*()}!{><;`=+]/', $value)){
			$session->message("Found Malicious Code. ".$value); redirect_to("customer.php");
		} 
	}
	...
	if(empty($_POST['password'])){
		$session->message("Enter Password");
	...
	if ($_POST['emp_name_1'] == "") {
		$session->message("Enter Name"); redirect_to("customer.php");
	} else {
	...
	if($company_reg->save()) {
		// Success
		$session->message("Customer Added Successfully.");
		redirect_to("customer.php");
	} else {
		// Failure
		//$session->message("Fail to Create New LC");
		$session->message("Fail to Create Customer.");
		redirect_to("customer.php");
	}
