...
<head>
<meta charset="utf-8" />
<title>Mill Data | Mahindraaccelo</title>
...
<div class="card-body">
<div class="al-com">
<div class="compl-lef"><h4>Mill Data</h4></div>
...
<?php
	...
	foreach($mill_reg as $mill_reg){
		...
		if(isset($_GET['date_range'])){
			...
			if($num > 0){
				$sql = "Select * from complaint where emp_id = '{$session->employee_id}' AND {$date_range} date_ != '0000-00-00' order by id DESC";
		...
		foreach($complaint as $complaint){
			...
			if($complaint->mill == $mill_reg->mill_name && $complaint->status == "Closed"){
			...
			if($complaint->mill == $mill_reg->mill_name && $complaint->status == "Closed"){
				...
				//echo $total_complaint_closed.",";
				//echo "<br />";
				//echo $total_complaint_closed.",".$total_complaint_tat.", ";
...
<?php 
	$total_total_complaint_tat += $total_complaint_tat; }
	
	if($total_total_complaint_tat > 0){
		$total_average_complaint_tat = $total_total_complaint_tat / $total_total_complaint_closed;
	}
?>
