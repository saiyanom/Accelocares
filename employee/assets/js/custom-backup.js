...
$(document).ready(function() {
	...
	if($(".id_source_val").val() == "Mill"){
		...
		$(".id_source .id_source_plant").addClass('btn-light');
	} else if($(".id_source_val").val() == "Plant"){
		$(".select_mill").hide(); 
		$(".id_source .btn").removeClass('btn-light');		
		$(".id_source .btn").removeClass('btn-primary');		
		$(".id_source .id_source_mil").addClass('btn-light');
...
/* Raise Complaint Form Submition *************************************************************/
	$("#create_complaint").click(function() {	
		...
		if ($("#invoice_number").val() != "") {
		  	...
		  	if(invoice_number.length != 12){
				$("#invoice_number").css("border","1px solid #f00");
		  		//alert("Enter 12 digit Invoice Number");
		  		$(".invoice_number_msg").show(); $(".invoice_number_msg").html("Enter 12 digit Invoice Number");
		  		return false;
		  	} else { $("#invoice_number").css("border","1px solid #dee2e6"); $(".invoice_number_msg").hide(); $(".invoice_number_msg").html("");} 
		} 
		...
		*/
		...
		if ($("#pl_name").val() == "Other") {
			...
			if ($("#pl_name_2").val() == "") {
				...
				if ($("#pl_email").val() == "") {
					...
					return false;
				} else { $("#pl_email").css("border","1px solid #dee2e6"); $(".pl_email_msg").hide(); $(".pl_email_msg").html("");}  
		...
		if($('#product_img_1').val() != ''){
			...
			if(file_size>300000) {
				$(".product_img_1_msg").show(); $(".product_img_1_msg").html("File size is greater than 300kb");
				img_error += 1;
			} 
		} 
		...
		if($('#product_img_2').val() != ''){
			...
			if(file_size>300000) {
				$(".product_img_2_msg").show(); $(".product_img_2_msg").html("File size is greater than 300kb");
				img_error += 1;
			} 
		} 
		...
		if($('#product_img_3').val() != ''){
			...
			if(file_size>300000) {
				$(".product_img_3_msg").show(); $(".product_img_3_msg").html("File size is greater than 300kb");
				img_error += 1;
			} 
		} 
...
/* Create CAPA Note *************************************************************/
	$("#save_capa").click(function() {
		...
		$("#create-capa input").each(function() {
