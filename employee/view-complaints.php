<?php 
	...
	if(isset($_GET['id']) && !empty($_GET['id'])){
		...
		if($complaint_reg->emp_id != $session->employee_id){
			$session->message("Complaint {$complaint_reg->ticket_no} is not Assigned to you");
			redirect_to("my-complaints.php");
		}
...
<div class="">
    ...
    else if($message == "Files updated Successfully"){echo "bg-success";}   
		else {echo "bg-danger";}?>" 
...
<div class="cb-block">
	...
	<div id="comp_raised_1" <?php echo "style='display:none;'"?>>
        ...
        <li class="resit-act1">
          <input type="text" id="size" name="size" placeholder="Size" value="<?php echo $complaint_reg->size; ?>" class="form-control">
          <span class="err_msg size_msg"></span>
			...
			<ul class="rais-block">
				<li class="resit-act1">
					<input type="text" readonly id="invoice_date" name="invoice_date" placeholder="Invoice Date" value="<?php echo date( 'd-m-Y', strtotime($complaint_reg->invoice_date)); ?>" class="form-control date-picker">
	...
	<div id="comp_raised_2">
		...
		<div class="cmp-lf"><h2><a class="btn btn-default back-btn" href="my-complaints.php"><i class="mdi mdi-keyboard-backspace"></i></a> Complaint details - <?php echo $complaint_reg->company_name; ?>   |   Complaint No. <?php echo $complaint_reg->ticket_no; ?></h2></div>
		<div class="clerfix"></div>
		<div class="tabobox">
		<div class="tabobox1">
...
<div class="onlock">
	...
	<div id="id_source_1" <?php if($complaint_reg->client_contacted == "Yes"){echo "style='display:none;'";}?>>
		...
		<div class="idf-sort-lft"><p>Identify the Source</p></div>
			<div class="idf-sort-rig onlock-2-acbtn id_source">
				<input type="button" class="btn <?php if($complaint_reg->identify_source == "Plant"){echo "btn-primary";} else if($complaint_reg->identify_source == "Mill"){echo "btn-light";} else {echo "btn-light";}?> id_source_plant" value="Plant" />   
				
        <input type="button" class="btn <?php if($complaint_reg->identify_source == "Mill"){echo "btn-primary";} else if($complaint_reg->identify_source == "Plant"){echo "btn-light";} else {echo "btn-light";}?> id_source_mil" value="Mill" />
				

        <input type="button" class="btn <?php if($complaint_reg->identify_source == 'Other'){echo "btn-primary";} else {echo "btn-light";} ?> id_source_other" value="Other" />
    ...
    <div class="idf-sort other_source_div" style="display: none;">
      <div class="idf-sort-lft"><p>Other source</p></div>
      <div class="idf-sort-rig">
        
        <input type="text" name="other_source"  class="form-control form-controlws" value="<?php echo $complaint_reg->other_source; ?>" placeholder="Enter other source">
        
        <span class="err_msg other_source_msg"></span>
      </div>
      <div class="clerfix"></div>
    </div>
		...
		<div class="idf-sort">
		<div class="idf-sort-lft"><p>&nbsp;</p></div>
		<div class="idf-sort-rig log-btn"><input type="submit" id="identify_the_source" name="identify_the_source" class="btn btn-primary2"  value="Submit" /></div>
		<div class="clerfix"></div>
		</div>
		</form>
	</div>
...
<div class="onlock" <?php if($complaint_reg->client_contacted == "" || $complaint_reg->client_contacted == "No"){echo "style='display:none'";} ?>>
	...
	<div id="request_visit_1" <?php if($complaint_reg->request_visit == "Yes" || $complaint_reg->request_visit == "No"){echo "style='display:none;'";}?>>
		...
		<div class="idf-sort">
		<div class="idf-sort-lft"><p>&nbsp;</p></div>
		<div class="idf-sort-rig log-btn"><input type="submit" id="request_a_visit" name="request_a_visit" class="btn btn-primary2"  value="Submit" /></div>
		<div class="clerfix"></div>
		</div>
		</form>
	</div>
...
<div class="onlock" <?php if(!empty($complaint_reg->request_visit) != "Yes" && $complaint_reg->request_visit != "No"){echo "style='display:none'";} ?>>
	...
	<form action="<?php echo "raised-complaint-db.php?id=".$_GET['id']; ?>" method="post" enctype="multipart/form-data" autocomplete="off">
		...
		<div class="onlock-2 visit_yes" <?php if($complaint_reg->visit_done != "Yes"){echo "style='display:none;'";}?>>
			<div class="onfod">
				<div class="input-group">
					...
					<div class="custom-file">
						<input type="hidden" value="<?php if(!empty($complaint_reg->mom_document)){echo "Yes";} ?>" id="mom_document_txt" name="mom_document_txt">
						<input type="file" name="mom_document" class="custom-file-input" id="mom_document" value="<?php if(!empty($complaint_reg->mom_document)){echo $complaint_reg->mom_document;}?>">
				...
				<span class="sdcon">(Please use .JPG or .PDF file format upto 25MB)</span>
			...
			<p class="colost2"><strong class="colost2">Upload Plant Images </strong><span class="sdcon">(Use minimum 2 images in .JPG or .PDF file format upto 25MB)</span></p>
				...
				<div class="plant_img">
					<div class="input-group plant-img-row plant_img_row_1">
						<div class="input-group">
							...
							<div class="custom-file">
								<input type="text" name='plant_img_txt_1' class="custom-file-input" id="plant_img_txt_1" value="<?php if(!empty($complaint_reg->plant_img_1)){echo $complaint_reg->plant_img_1;} ?>">
					...
					<div class="input-group plant-img-row plant_img_row_2">	
						<div class="input-group">
							<div class="input-group-prepend">
								<span class="input-group-text clear_img clear_img_2" title="Clear Image">X</span>
							</div>
							<div class="custom-file">
								<input type="text" name='plant_img_txt_2' class="custom-file-input" id="plant_img_txt_2" value="<?php if(!empty($complaint_reg->plant_img_2)){echo $complaint_reg->plant_img_2;} ?>">
					...
					<div class="input-group plant-img-row plant_img_row_3">		
						<div class="input-group">
							<div class="input-group-prepend">
								<span class="input-group-text clear_img clear_img_3" title="Clear Image">X</span>
							</div>
							<div class="custom-file">
								<input type="text" name='plant_img_txt_3' class="custom-file-input" id="plant_img_txt_3" value="<?php if(!empty($complaint_reg->plant_img_3)){echo $complaint_reg->plant_img_3;} ?>">
		...
		<div class="onlock-1 visit_yes" <?php if($complaint_reg->visit_done != "Yes"){echo "style='display:none;'";}?>><p>Complaint Status</p></div>
		<div class="onlock-2 visit_yes" <?php if($complaint_reg->visit_done != "Yes"){echo "style='display:none;'";}?>>
			<div class="oldsercj">
		...
		<div class="idf-sort">
		<div class="idf-sort-lft"><p>&nbsp;</p></div>
		<div class="idf-sort-rig log-btn"> &nbsp; &nbsp; <input type="submit" id="visit_a_done" name="visit_a_done" class="btn btn-primary2"  value="Submit" /></div>
		<div class="clerfix"></div>
		</div>
	</form>
	</div>
			...
			<div class="onlock" style="border:none">
				...
				<div class="">
					...
					<div class="onlock-cov2">
						<div class="btspa">
							<div class="form-group mb-0 text-center log-btn">
								<input type="submit" id="complaint_a_accepted" name="complaint_a_accepted" class="btn btn-primary2"  value="Submit"/>
							</div>
						</div>
					</div>
					<div class="clerfix"></div>
...
<div class="onlock" <?php 
	 if($complaint_reg->create_approval_note == ""){ echo "style='display:none'"; }
	 if($complaint_reg->create_approval_note == "Yes" && $complaint_reg->emp_status == "Approval Note Submitted"){ 
...
?>>
	...
	<div class="onlock-2" <?php if($complaint_reg->create_approval_note == ""){echo "style='display:none;'";} ?>>
		<div class="custom-control custom-checkbox arsalin">
			<input type="checkbox" <?php if(!empty($complaint_reg->capa) || !empty($complaint_reg->capa_document)){echo " Checked ";} if($complaint_reg->capa_qa == "Yes"){echo " disabled ";} ?> class="custom-control-input" id="capa_check">
...
?>>
	...
	<div id="capa_1" <?php if(!empty($complaint_reg->capa) || !empty($complaint_reg->capa_document)){echo "style='display:none;'";}?>>
		...
		<div class="onlock-2">
			<a class="onlock-link " href="#" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#create-capa">Create Document</a><div class="clerfix"></div>
		</div>
		<div class="clerfix"></div>
		...
		<form action="<?php echo "raised-complaint-db.php?id=".$_GET['id']; ?>" method="post" enctype="multipart/form-data" autocomplete="off">
			...
			<span>(Please use .JPG, .PNG or .PDF file format upto 25MB)</span>
			
			<div class="clerfix"></div>
			<div class="btspa">
				<div class="form-group mb-0 text-center log-btn">
					<input type="submit" id="upload_capa_document" name="upload_capa_document" class="btn btn-primary2"  value="Submit" />
				</div>
			<div class="clerfix"></div>
			</div>
...
<!--
<div class="onlock" <?php echo "style='display:none'"; ?>>
	<div class="onlock-1"><p class="colost1">&nbsp;</p></div>
	<div class="onlock-2">
	...
	<ul class="fpall">
		...
		<li class="fp-con2">
			<div class="row">	
				...
				<div class="col col-sm-7">
					<input type="text" id="total_qty_rejc" name="total_qty_rejc" value="" class="form-control form-p-box-bg2 app_note_number">
				</div>
			</div>
		</li>
		<div class="clerfix"></div>
...
<div class="rst-full">
	<ul class="fpall">
		...
		<input type="text" id="basic_sale_price" name="basic_sale_price" value="" class="form-control form-p-box-bg2 sphf app_note_number"></li>
		<div class="clerfix"></div>
	</ul>
	<ul class="fpall">
...
<div class="rst-full">
	...
	<ul class="fpall">
		...
		<li class="fp-con02"><span class="padd10">&nbsp; &nbsp; &nbsp; &nbsp; Rs.</span>  
		<input type="text" id="debit_note_sal_rate_sale_value" name="debit_note_sal_rate_sale_value" value="" class="form-control form-p-box-bg2 sphf app_note_number"></li>
		<div class="clerfix"></div>
	</ul>
  ...
  <ul class="fpall">
    <li class="fp-con01">
      ...
      <input type="text" class="form-control optional_field" name="d_sale_value" placeholder="Sale value" style="width: 200px; float: left;margin-right: .75rem;"></li>
    <div class="clerfix"></div>
  </ul>
...
<ul class="fpall-z">
	...
	<li class="fp-con2-z"><input type="text" id="document_no" name="document_no" value="<?php echo $complaint_reg->invoice_number; ?>" class="form-control form-p-box-bg1"></li>
</ul>
...
<ul class="fpall">
	...
	<li class="fp-con2">
		<input id="date_issue" type="text" name="date_issue" value="<?php echo date( 'd-m-Y', strtotime($complaint_reg->invoice_date)); ?>" readonly class="form-control">
	</li>
<div class="clerfix"></div></ul>
...
<ul class="fpall">
	...
	<li class="fp-con2">
		<select id="reviewed_by" name="reviewed_by" class="form-control select2" data-toggle="select2">
			<?php
				$employee_reg = EmployeeReg::find_all();
				foreach($employee_reg as $employee_reg){
					echo "<option value='{$employee_reg->emp_name}'>{$employee_reg->emp_name}</option>";
...
<ul class="fpall">
	...
	<li class="fp-con2">
		<input id="feedback_date" type="text" name="feedback_date" readonly class="form-control date-picker">
	</li>
	<div class="clerfix"></div>
</ul>
...
<ul class="fpall">
	...
	<li class="fp-con2">
		<input id="reviewed_date" type="text" name="reviewed_date" readonly class="form-control date-picker">
	</li>
	<div class="clerfix"></div>
</ul>
...
<ul class="ulsp1">
	...
	<li class="ulsp1-r"><input type="text" id="problem_when" name="problem_when" readonly class="form-control date-picker"></li>
	<div class="clerfix"></div>
</ul>
...
<div class="form-p-box form-p-box-bg2 border-bottom-st">
	<div class="sectp-fom">
		...
		<div class="row">
			...
			<div class="col col-sm-6"><input type="text" id="correction_when" name="correction_when" placeholder="When" readonly class="form-control form-p-box-bg1 mrig10 date-picker"></div>
		</div>
		<div class="clerfix"></div>
	</div>
	<div class="clerfix"></div>
...
<div class="form-p-box form-p-box-bg2 border-bottom-st">
	<div class="sectp-fom">
		...
		<textarea class="form-control mrig10 form-p-box-bg1" id="correction_action_remark" name="correction_action_remark" placeholder="Remarks" rows="3"></textarea>
		<div class="row">
			<div class="col col-sm-6 correction_who correction_action_who">
				<select id="correction_action_who" name="correction_action_who[]" class="form-control select2 select2-multiple form-p-box-bg1 mrig10" data-toggle="select2" multiple="multiple">
					<?php
						$employee_reg = EmployeeReg::find_all();
						foreach($employee_reg as $employee_reg){
							echo "<option value='{$employee_reg->emp_name}'>{$employee_reg->emp_name}</option>";
			...
			<div class="col col-sm-6"><input type="text" id="correction_action_when" name="correction_action_when" placeholder="When" readonly class="form-control form-p-box-bg1 mrig10 date-picker"></div>
		</div>
		<div class="clerfix"></div>
	</div>
	<div class="clerfix"></div>
...
<div class="form-p-box form-p-box-bg2 border-bottom-st">
	<div class="sectp-fom">
		...
		<textarea class="form-control mrig10 form-p-box-bg1" id="verify_remark" name="verify_remark" placeholder="Remarks" rows="3"></textarea>
		<div class="row">
			<div class="col col-sm-6 correction_who verify_who">
				<select id="verify_who" name="verify_who[]" class="form-control select2 select2-multiple form-p-box-bg1 mrig10" data-toggle="select2" multiple="multiple">
					<?php
						$employee_reg = EmployeeReg::find_all();
						foreach($employee_reg as $employee_reg){
							echo "<option value='{$employee_reg->emp_name}'>{$employee_reg->emp_name}</option>";
			...
			<div class="col col-sm-6"><input type="text" id="verify_when" name="verify_when" placeholder="When" readonly class="form-control form-p-box-bg1 mrig10 date-picker"></div>
		</div>
		<div class="clerfix"></div>
	</div>
	<div class="clerfix"></div>
...
<div class="form-p-box form-p-box-bg2">
	<div class="sectp-fom">
		...
		<div class="row">
			<div class="col col-sm-6 correction_who prevent_who">
				<select id="prevent_who" name="prevent_who[]" class="form-control select2 select2-multiple form-p-box-bg1 mrig10" data-toggle="select2" multiple="multiple">
					<?php
						$employee_reg = EmployeeReg::find_all();
						foreach($employee_reg as $employee_reg){
							echo "<option value='{$employee_reg->emp_name}'>{$employee_reg->emp_name}</option>";
			...
			<div class="col col-sm-6"><input type="text" id="prevent_when" name="prevent_when" placeholder="When" readonly class="form-control form-p-box-bg1 mrig10 date-picker"></div>
		</div>
		<div class="clerfix"></div>
	</div>
	<div class="clerfix"></div>
...
<!-- Create New/edit Approval Note -->
<div class="modal fade form-pop" id="edit_approval_note" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content"></div> <!-- end modal-content-->
  </div> <!-- end modal dialog-->
</div>
...
<script>
  ...
  $('.edit_approval_note').click(function() {
    var data_id = $(this).attr('data-id');

    console.log(data_id);

    $("#edit_approval_note .modal-content").load("edit-approval-note.php?id="+data_id);
  });
	...
	$('.download_capa').click(function(e) {
		var data_id 	= $(this).attr('data-id');
		var data_type 	= $(this).attr('data-type');
		var api = "download-capa.php?doc="+data_type+"&id="+data_id;
	...
	$(document).ready(function() {
		...
		$("#capa_check, #capa_check2").click(function() {	
			if ($('#capa_check, #capa_check2').is(":checked")) {
				$('#create_capa').show();
			} else {
				$('#create_capa').hide();
			}
		...
		$('#create-note input').blur(function(){
			...
			sales_value 		= (parseFloat(total_qty_rejc) * parseFloat(basic_sale_price)).toFixed(2);
			$("#sales_value").val(sales_value);
			
			cgst 	= (parseFloat(sales_value) / 100) * $(".cgst").val();
			sgst 	= (parseFloat(sales_value) / 100) * $(".sgst").val();
			cgst 	= cgst.toFixed(2);
			sgst 	= sgst.toFixed(2);
			
			$("#cgst").val(parseFloat(cgst));
			$("#sgst").val(parseFloat(sgst));
			...
			credit_note_iss_cust = parseFloat(credit_note_iss_cust);
			$("#credit_note_iss_cust").val(credit_note_iss_cust.toFixed(2));
			//$("#credit_note_iss_cust").val(999);
			console.log(sales_value);
			console.log(cgst);
			console.log(sgst);
			console.log(cost_inc_customer);
			console.log(salvage_value);
