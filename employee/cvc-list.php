<?php 
	...
	foreach ($_POST as $key => $value) {
		if (preg_match('/[\'"\^%*()}!{><;`=+]/', $value)){
			$session->message("Found Malicious Code."); redirect_to("cvc-list.php");
		} 
	}
...
<?php
	...
	if(isset($_GET['meet_status']) || isset($_GET['date_range'])){		
		...
		if($num > 0){
			$sql = "Select * from cvc_meeting where emp_id = '{$session->employee_id}' AND {$meet_status} {$date_range} date_ != '0000-00-00' order by id DESC";
