...
<table id="my-complaints" class="table dt-responsive nowrap basic-datatable" width="100%">
	...
	<?php
		...
		if(isset($_GET['company'])){
			...
			//echo $num."<hr />";
			if($num > 0){
				$sql = "Select * from complaint where {$company} {$status } {$date_range} {$department} {$complaint_type} {$sub_complaint_type} {$identify_source} emp_id = {$employee->id}  order by id DESC";
			} else {
				$sql = "Select * from complaint where {$status } emp_id = {$employee->id} Order by id DESC";
			}			
		} else {
			$lower_aging = 0;
			$upper_aging = 20;
