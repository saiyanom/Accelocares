	...
	if(isset($_POST['identify_the_source'])){
		...
		if($complaint->save()) {
			...
			if($_POST['id_source'] == "Mill"){
				...
				//$mail->addAddress('{$millReg->email_1}', '{$millReg->mill_name}');   // Add a recipient
				//$mail->addAddress('{$millReg->email_2}', '{$millReg->mill_name}');   // Add a recipient
				//$mail->addAddress('{$millReg->email_3}', '{$millReg->mill_name}');   // Add a recipient
				...
				<p>We have received a complaint with Ticket ID - {$complaint->ticket_no} from {$complaint->company_name}</p>
				<p>Would appreciate if you could study the registered complaint & let us know the next steps.</p> 


				<p>Hoping to keep in touch.</p>
...
//   ----------------- Meeting Appointment  --------------------------------------------
	...
	if(isset($_POST['complaint_meeting'])){
		...
		if($complaint_meeting->save()) {
			...
			$mail->addAddress('salvi.suvarna@mahindra.com','Salvi Suvarna');
			$mail->addAddress('gajelli.lalita@mahindra.com','Lalita Gajelli');
			...
			<p>As per our last conversation, one of our officials will be paying a visit to your site on {$meeting_date_2} to work towards a solution for the feedback registered.</p>
...
//   ----------------- visit_a_done --------------------------------------------
	...
	if(isset($_POST['visit_a_done'])){
		...
		if($complaint->save()) {
			...
			<p>In case you would like to have a look at it, here’s a link for the same - (Link)</p>
	...
	if(isset($_POST['complaint_a_accepted'])){
		...
		if($complaint->save()) {
			...
			if($_POST['complaint_accepted'] != "Decision Pending"){
				...
				$mail->Priority = 1;
				$mail->setFrom('crm.accelo@emailmahindra.com', 'Mahindra Accelo');
				$mail->addAddress('{$empReg->emp_email}', '{$empReg->emp_name}');   // Add a recipient
				//$mail->addAddress($complaint->pl_email, $complaint->pl_name);   // Add a recipient
				//$mail->addCC('cc@example.com');
				//$mail->addBCC('bcc@example.com');
				...
				<p>Dear {$empReg->emp_name} </p> <br />

				<p>The decision for the complaint registered under the Ticket ID- {$complaint->ticket_no} has been taken.</p>
				<p>We would like to confirm that the action to be taken for the same is: Rework and send back.</p>
				<p>We’re glad to have a dedicated & skilled team working towards the comfort of our customers. </p>
...
//   ----------------- Approval Note Create --------------------------------------------
	...
	if(isset($_POST['create_a_note'])){
		...
		if($appr_note->save()) {
			...
			<p>Dear {$emp_location->emp_name} </p> <br />
	...
	if(isset($_POST['save_capa'])){
		...
		if($capa->save()) {
			...
			if(!$mail->send()) {
			    //echo 'Message could not be sent. <br />';
			    //echo 'Mailer Error: ' . $mail->ErrorInfo;
			    $session->message("CAPA Created Successfully, Message could not be sent.");
				redirect_to("view-complaints.php?id=".$_GET['id']);
			} else {
			    //echo 'Message has been sent';
	...
	if(isset($_POST['upload_capa_document'])){
		...
		if($complaint->save()) {
			...
			if(!$mail->send()) {
			    //echo 'Message could not be sent. <br />';
			    //echo 'Mailer Error: ' . $mail->ErrorInfo;
			    $session->message("CAPA Document Uploaded Successfully, Message could not be sent.");
				redirect_to("view-complaints.php?id=".$_GET['id']);
			} else {
			    //echo 'Message has been sent';
