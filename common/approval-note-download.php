...
if(isset($_GET['id']) && !empty($_GET['id'])){
  ...
  //approver remarks new
  $aprvr_remark_html = '';
  $remarks = ApprovalRemark::find_by_note_id($approval_note->id);

  if($remarks){
    $aprvr_remark_html = '<p class="colrred_black"><strong>Approver Remark:</strong></p>
      <table width="100%" border="1" style="border: 1px solid #000;" cellspacing="0" cellpadding="5">';
 
    foreach ($remarks as $key => $value) {
      $aprvr_remark_html .= '<tr>
                <td width="15%"><strong>'.$value->emp_name.'</strong></td>
                <td width="85%"><strong>'.$value->remark.'</strong></td>
            </tr>';
    } 
    $aprvr_remark_html .= '</table>'; 
 }

 // calculate total debit value
    $qty_acpt_steel_mill = explode(" ", trim($approval_note->qty_acpt_steel_mill));
    $qty_scrp_auc_serv_cent = explode(" ", trim($approval_note->qty_scrp_auc_serv_cent));
    $qty_dlv_customer = explode(" ", trim($approval_note->qty_dlv_customer));

    $qty_acpt_steel_mill      = isset($qty_acpt_steel_mill[0]) ? $qty_acpt_steel_mill[0] : 0;

    $qty_scrp_auc_serv_cent      = isset($qty_scrp_auc_serv_cent[0]) ? $qty_scrp_auc_serv_cent[0] : 0;

    $qty_dlv_customer      = isset($qty_dlv_customer[0]) ? $qty_dlv_customer[0] : 0;

    $qty_debit            = $qty_acpt_steel_mill + $qty_scrp_auc_serv_cent + $qty_dlv_customer;

    $debit_note_sal_rate_sale_value = $approval_note->debit_note_sal_rate_sale_value;
    $value              = $qty_debit * $debit_note_sal_rate_sale_value;
    $value              = round($value, 2);

    $loss_cgst  = ($value / 100) * round($approval_note->lcgst_percent, 2);
    $loss_sgst  = ($value / 100) * round($approval_note->lsgst_percent, 2);
...
<table border="0" cellpadding="2" align="center"   cellspacing="0" width="90%" style="border:1px solid;font-family:Arial, Helvetica, sans-serif; font-size:10px; color:#000">
  <tbody>
    <tr style="" class="row0">
      <td  style="text-align:center;border:1px solid ; border-bottom:2px solid ; font-size:12px "  class="column0 style2 s style4" colspan="4">
        <strong>MAHINDRA ACCELO LTD. / MASL / MSSCL<br />
        Credit Note Approval</strong>
      </td>
    </tr>
    ...
    <tr class="row4">
      <td style="border-left:1px solid;" class="column0 style13 s">Plant</td>
      <td class="column1 style15 s">'. $complaint->product.' | '.$complaint->business_vertical.' | '.$complaint->plant_location.'</td>
      <td class="column2 style15 null" align="right"></td>
      <td style="border-right:1px solid;" class="column3 style16 null" align="right"></td>
    </tr>
    <tr class="row5">
    ...
    <tr class="row24">
      <td style="border:1px solid" class="column0 style1 s" colspan="2">Salvage Value '. $approval_note->salvage_value_txt.'</td>
      <td style="border:1px solid" class="column2 style31 s" align="right">Rs.</td>
    ...
    <tr class="row36">
      <td style="border:1px solid" class="column0 style1 s" colspan="2">Debit Note (purchase price/t) / Salvage rate / Sale Value '. $approval_note->debit_salvage_sale_txt.'
        <hr> 
        Purchase price/t : '.$approval_note->d_purchase_price.' <br>
        Salvage rate : '.$approval_note->d_salvage_rate.' <br>
        Sale value  : '.$approval_note->d_sale_value.' <br>
      </td>
      <td style="border:1px solid" class="column2 style56 s" align="right">Rs.<hr><br><br><br></td>
      <td style="border:1px solid" class="column3 style28 n" align="right">'. $approval_note->debit_note_sal_rate_sale_value.'<hr><br><br><br></td>
    </tr>
    ...
    <tr class="row41">
      ...
      <td style="border:1px solid" class="column3 style58 null" align="right">'. $total_debit_value.'</td>
    ...
    <tr class="row57">
      <td style="border:1px solid" class="column0 style53 s" colspan="2">Other Realisation (if applicable) </td>
      <td style="border:1px solid" class="column2 style56 s" align="right"><strong style="float: left;">(g)</strong> Rs.</td>
      <td style="border:1px solid" class="column3 style77 n" align="right">'. $approval_note->other_realisation.'</td>
    </tr>

    <tr class="row59">
      <td style="border:1px solid" class="column0 style70 s" colspan="2"><strong>Net Loss / (-) Net Profit</strong> </td>
      <td style="border:1px solid" class="column2 style65 s" align="right"><strong style="float: left;">(e-f-g)</strong> Rs.</td>
  ...
  if( $approval_note->approver_remark ){
      $html  .= '<p class="colrred_black"><strong>Approver Remark</strong></p>
        '. $approval_note->approver_remark.'<br />
        <hr />';
  }
  
   $html  .= $aprvr_remark_html.'
        <p>
...
<i class="signature"> This is a computer-generated document. No signature is required. 09 </i>
...
$doc_name = str_replace(' ', '', $approval_note->billing_doc_no); // Replaces all spaces with hyphens.
$doc_name = preg_replace('/[^A-Za-z0-9\-]/', '_', $doc_name); // Removes special chars.
$doc_name = preg_replace('/-+/', '-', $doc_name); // Replaces multiple hyphens with single one.

$pdfname = "approval-note-".$doc_name.".pdf";
